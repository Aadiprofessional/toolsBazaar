...
const CartItem = ({ item, onRemoveItem, onUpdateQuantity }) => {
  ...
  const discountedPrice = item.product.price ? item.product.price - (item.product.price * (item.product.additionalDiscount / 100)) : 0;
  ...
  const truncatedDescription = item.product.description ? 
    (item.product.description.length > descriptionLimit ? 
      ...
      <div className={`right-part ${isExpanded ? 'expanded' : ''}`} onClick={handleToggleExpand}>
        ...
        <div className="right-part-calculations">
          <p className="right-part-text">
              <strong className="detail-title">Price:</strong> ₹{totalPrice.toFixed(2)}
          </p>
          <p className="right-part-text">
